...
class GetProductsLevelOneUseCase @Inject constructor(
    private val repository: ProductRepository
) {
    operator fun invoke(forceRefresh: Boolean = false): Flow<DataResult<List<ProductModel>>> {
        return repository.getProductsLevelOne(forceRefresh)
    }
}